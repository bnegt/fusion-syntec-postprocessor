...
description = "LNC Kroloftet";
vendor = "LNC";
vendorUrl = "http://biobonden.se";
legal = "";
certificationLevel = 2;
minimumRevision = 40783;

longDescription = "Post för LNC på kroloftet";
...
properties = {
  dustCover: true, // Lowers the dust cover by default
  materialHolder: false, // Material holder off by default
  dustVacuum: true, // Dust vqaacuum motor on by default
  tableVacuum: true, // Enable the table sucktion vacuum by default
  ...
  retractPins: true, // Retract position pins before running program
  separateWordsWithSpace: true // specifies that the words should be separated with a white space
};

// user-defined property definitions
propertyDefinitions = {
  tableVacuum: {title: "Table sucktion vacuum pump",description: "Turn vacuum on or off", group:0, type: "boolean"},
  materialHolder: {title: "Material holder and dust collector",description:"Raise or lower the dust vacuum and material holder",group: 0, type:"boolean"},
  dustVacuum: {title:"Dust vacuum motor on",description:"Turn dust vacuum motor on or off",group: 0,type:"boolean"},
  dustCover: {title: "Dust cover down",description:"Raise or lower the dust cover", group: 0, type:"boolean"},
  ...
  retractPins: {title:"Retract positions pins", description:"Safety precausion, retract pins before running the program.", type:"boolean"},
...
function onOpen() {
  ...
  if(properties.retractPins == true)
    writeBlock(onPassThrough("M9"));


  if(properties.dustCover == true)
    writeBlock(onPassThrough("M141"));
  else
    writeBlock(onPassThrough("M140"));

  if(properties.materialHolder == true)
      writeBlock(onPassThrough("M86"));
  else
      writeBlock(onPassThrough("M87"));

  if(properties.dustVacuum == true)
      writeBlock(onPassThrough("M71"));
  else
      writeBlock(onPassThrough("M72"));

  if(properties.tableVacuum == true)
      writeBlock(onPassThrough("M10"));
  else
      writeBlock(onPassThrough("M11"));
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
      ...
      if (workOffset != currentWorkOffset) {
        ...
        //Added a 5 second delay for spindle to ramp up speed
